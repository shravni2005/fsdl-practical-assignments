          ...
          {TRAVEL_PACKAGES.map((pkg) => (
            <div key={pkg.id} className="group relative">
              <div className="relative aspect-[3/4] overflow-hidden rounded-[2.5rem] mb-10 shadow-2xl transition-all duration-700 group-hover:-translate-y-4">
                <img src={pkg.imageUrl} alt={pkg.title} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
                ...
                <div className="absolute bottom-10 left-10 right-10">
                   <Link to={`/book/${pkg.id}`} className="w-full block text-center bg-white text-slate-950 py-5 rounded-[1.25rem] text-xs font-black uppercase tracking-[0.2em] shadow-2xl opacity-0 translate-y-6 transition-all duration-500 group-hover:opacity-100 group-hover:translate-y-0 hover:bg-indigo-50">
                    View Details
                  </Link>
                </div>
              </div>
              <div className="px-4">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-3xl font-black text-white">${pkg.price} <span className="text-slate-500 text-[10px] ml-1.5 uppercase font-black tracking-widest">USD</span></span>
                  <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white group-hover:bg-indigo-500 group-hover:border-indigo-500 transition-all">
